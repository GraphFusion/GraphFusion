...
fn col(relation: &str, name: &str) -> Expr {
    Expr::Column(Column::new(Some(TableReference::bare(relation)), name))
}
fn memory(schema: SchemaRef, batches: Vec<RecordBatch>) -> Result<LogicalPlanBuilder> {
...
/// Materialize once before a write barrier, retaining GQL variable qualifiers.
async fn freeze(
    ...
    for input in batches {
        ...
        if let Some((_, ids)) = &ids {
            let slice = ids
                .get(offset..offset + input.num_rows())
                .ok_or_else(|| Error::Corrupt("write input cardinality changed".into()))?;
            values.push(Arc::new(UInt64Array::from(slice.to_vec())));
        }
...
async fn patch(
    ...
    let patch = plan.clone().project(expressions)?.distinct()?.build()?;
